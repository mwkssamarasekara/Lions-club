...
async function getGalleryCategories() {
  try {
    ...
    if (list.length === 0) {
      const defaults = ['Service', 'Education', 'Health', 'Donations', 'Meetings'];
      const seeded = [];
      for (const cat of defaults) {
        const docRef = await addDoc(collection(db, 'galleryCategories'), { name: cat });
        seeded.push({ id: docRef.id, name: cat });
      }
      return seeded;
    }
    return list;
